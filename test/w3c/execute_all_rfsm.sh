#/bin/bash
# Executes all "rfms" files. "RFSM" is the binary format written by the rfsm serializer.
# See "tests_to_rfsm.sh".
...
for TEST_FILE in rfsm/*.rfsm; do
  ...
  $RFSM_BIN -includePaths dependencies/scxml test_config.json "$TEST_FILE" 1>"rfsm_logs/$TEST_NAME.log" 2>&1
  if [ $? -eq 0 ]; then
      OK_COUNT=$(( OK_COUNT + 1 ))
